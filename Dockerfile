...
RUN gradle build --exclude-task test  --no-daemon  --exclude-task integrationTest   -Pquartz -Pprod -Papi-docs bootJar
...
COPY --from=build /home/gradle/src/build/libs/baseapi-0.0.1-SNAPSHOT.jar /app/spring-boot-application.jar
